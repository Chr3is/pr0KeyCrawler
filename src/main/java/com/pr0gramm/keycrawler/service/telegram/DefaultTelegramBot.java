...
public class DefaultTelegramBot extends BaseTelegramBot {
    ...
    private boolean checkInput(Trio<Update, Ability, String[]> trio) {
        ...
        if (!isOk) {
            silent.send(getLocalizedMessage(CHECK_INPUT_FAIL, AbilityUtils.getUser(trio.a()).getLanguageCode(),
    ...
    private boolean checkLocality(Trio<Update, Ability, String[]> trio) {
        ...
        if (!isOk) {
            silent.send(getLocalizedMessage(CHECK_LOCALITY_FAIL, AbilityUtils.getUser(trio.a()).getLanguageCode(),
    ...
    private boolean checkPrivacy(Trio<Update, Ability, String[]> trio) {
        ...
        if (!isOk) {
            silent.send(getLocalizedMessage(CHECK_PRIVACY_FAIL, AbilityUtils.getUser(trio.a()).getLanguageCode()),
                    getChatId(trio.a()));
        }
        return isOk;
    }
